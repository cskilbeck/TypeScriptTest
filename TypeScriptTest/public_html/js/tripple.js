...
window.onload = function () {
    ...
    var Player = glib.Class({ inherit$: glib.Drawable,
        ...
        exit: function(time, deltaTime) {
            ...
            if (this.stateTime > 0.5) {
                ...
                t = glib.Util.ease2(Math.min(1, this.stateTime * 4), 6);
        ...
        play: function(time, deltaTime) {
            var dt = Math.min(1000/10, deltaTime * 1000),
                of = this.flip,
                mask,
                i,
                u,
        ...
        onUpdate: function(time, deltaTime) {
            if (this.visible) {
                ...
                if (this.age >= 2) {
                    ...
                    this.setScale(1 + this.age / 2);
                    this.transparency = 255 - (this.age / 2) * 255;
                }
            }
        }
    });
    ...
    var Game = glib.Class({ inherit$: glib.Drawable,
        ...
        init: function() {
            ...
            this.instructions = new Instructions("Press space to move & jump", this.font).setPivot(0.5, 0.5).setPosition(this.width / 2, this.height / 2);
    ...
    playfield = new glib.Playfield({
        width: 852,
        height: 480,
        backgroundColour: "rgb(32, 128, 48)",
        autoCenter: true,
